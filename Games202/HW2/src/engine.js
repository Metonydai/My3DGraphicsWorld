...
let currentRotationMatrix = mat4.create();
let lastTime = 0;

let globalWhiteTexture = null;

function initGlobalTextures(gl) {
	globalWhiteTexture = new Texture();
	globalWhiteTexture.CreateConstantTexture(gl, [1.0, 1.0, 1.0]);
}
...
async function GAMES202Main() {
	...
	function createGUI() {
		...
		const panelSettings = gui.addFolder('Scene Settings');
		panelSettings.add(guiParams, 'scene_rotation').name('Rotate Scene');
		panelSettings.add(guiParams, 'using_texture').name('Using Texture');
		panelSettings.open();
	}
